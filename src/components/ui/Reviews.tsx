...
function Review({ imgSrc, className, ...props }: ReviewProps) {
  return (
    <div
      className={cn(
        "animate-fade-in rounded-[2.25ren] bg-white p-6 opacity-0 shadow-xl shadow-slate-900/5",
        className
      )}
...
const animationDelay =
  POSSIBLE_ANIATION_DELAY[
    Math.floor(Math.random() * POSSIBLE_ANIATION_DELAY.length)
  ];
...
function ReviewGrid() {
  ...
  return (
    <div
      ref={containerRef}
      className=" relative -mx-4 mt-16 grid h-[49rem] max-h-[150vh] grid-cols-1 gap-8 px-4  overflow-hidden sm:mt-20 md:grid-cols-2 lg:grid-cols-3"
    >
      {IsInView ? (
        <>
          <ReviewColumn
